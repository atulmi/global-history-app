// ISS-007: My Notes page test suite
...
describe("My Notes Page", () => {
  ...
  it("6 — Sort by oldest", () => {
    ...
    cy.get('[data-testid="notes-table-row"]')
      .first()
      .should("contain", "Note A");
  });
  ...
  it("10 — Edit note from table", () => {
    ...
    cy.get('[data-testid="notes-table-row"]').should(
      "contain",
      "Updated content",
    );
  });
  ...
  it("12 — Split view — 2 countries", () => {
    openSelect("filter-country");
    cy.get('[role="listbox"]')
      .contains("Display notes for 2 countries")
      .click();
